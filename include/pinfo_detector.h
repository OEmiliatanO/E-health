...
public:
    ...
    std::string detect(patient_info_t& patient_info,Detector_Status_t& detector_status) override{
        ...
        int heart_rate = numerical_record["heart_rate"];
...
public:
    Detector() = default;
    Detector(std::vector<DetectorModule>& detectors_,patient_info_t& patient_info_):
        detectors{std::vector<DetectorModule>()},
        patient_info{patient_info_}
    {
        detectors.resize(detectors_.size());
        for(int i=0;i < detectors_.size() ; ++i){
            detectors[i] = detectors_[i];
        }
    }
    Detector(std::vector<DetectorModule>&& detectors_,patient_info_t& patient_info_):
    ...
    Detector& operator=(Detector& another){
        ...
        detectors=another.get_detectors();
        return *this;
    }
    //detect
    void detect(std::vector<std::string>& descs,std::vector<Detector_Status_t>& status){
        ...
        for(int i=0;i<status.size();++i){
            descs.emplace_back(detectors[i].detect(patient_info,status[i]));
        }
    }
    //Detectors Editting
    void append_detector(DetectorModule& detector){
    ...
    //Get and Set
    std::vector<DetectorModule> get_detectors(){
        return detectors;
    }
    ...
    void set_detectors(std::vector<DetectorModule>& detectors_){
        detectors = detectors_;
    }
    
private:
    std::vector<DetectorModule> detectors;
