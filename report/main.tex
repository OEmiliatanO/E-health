...
\begin{figure}[h]
    \centering
    \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_uml.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_main.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_search_patient.png}
  \caption{The prview of patient_info_layout}
  \label{fig:frontend_search_patient}
\end{figure}
The page firstly get input from textbox, and use \codeword{get_info_by_id()} to call serials of methods in \codeword{patient_info_t} to load info of the patient the doctor wants to search. Then, it will call \codeword{display_info()} to open a subprocess and pass the patient's info it gets. The subprocess can use a describe object to present these info.
\begin{figure}[h]
  \centering
  \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_patient_info.png}
  \caption{The prview of describe object}
  \label{fig:frontend_patient_info}
\end{figure}
The describe object will firstly process the info by using \codeword{process_info(String)}, put it into a suitable format and divide it into parts.
Then, it will present these information by using a scoll window, there are buttons to go prev and next pages, each pages present individual part of informations it just processed by \codeword{process_info(String)}.

\subsubsection*{Reservation_layout}
\begin{figure}[h]
  \centering
  \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_reservation_browse.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_reservation_edit.png}
...
\begin{itemize}
  \item \codeword{delete_resv():} Activated by the red trash can button. Will delete reservation the button belongs to from its own attributes, then will call \codeword{fix_page()} to refresh the embedded page.
  \item \codeword{chg_edit_resv():} Activated by the yellow pen button. The method will change current page to \codeword{EditResv} class, doctor can edit the reservation the button belongs to. After press the save button the data will be stored to that reseravtion and called \codeword{change_to_Reservatoin()} to return to original page.
\end{itemize}

\subsubsection*{Chat_layout}
\begin{figure}[h]
  \centering
  \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_chat_browse.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_chat_show.png}
  \caption{The prview of MailDisplay}
  \label{fig:frontend_chat_show}
\end{figure}
\begin{figure}[h]
  \centering
  \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_chat_reply.png}
  \caption{The prview of ReplyMail}
  \label{fig:frontend_chat_reply}
\end{figure}

\begin{itemize}
  \item \codeword{check_mail():} Activated by clicking the summary of that message. It will call a subprocess and passing needed arguments such as sender, date, message. Then the subprocess will display these information by \codeword{MailDisplay}.
  \item \codeword{delete_mail():} Activated by the red fire button. Will delete message the button belongs to, then will call \codeword{fix_page()} to refresh the embedded page.
  \item \codeword{delete_mail():} Activated by the yellow paper airplane button. It will also call a subprocess to let doctor write message to the sender. After pressing save button, the message will be send to the sender by using \codeword{reply} method that the mail have defined in medium(linebot)'s \codeword{Message}.
\end{itemize}

\subsubsection*{Prscribe_layout}
\begin{figure}[h]
  \centering
  \includegraphics[scale = 0.5]{asset/dcotor_gui/frontend_prescribe.png}
...
\begin{itemize}
  \item \codeword{update_scollview():} Activated by the preview button. Will load the textinput right of "Key Value:" text, process them into proper format and update the scroll window right of "Preview:" text. The reason to create such a function like this is cause associative values are strictly format, doctors can preview the result to check if there is something wrong by this method.
  \item \codeword{delete_mail():} Activated by the red fire button. Will delete message the button belongs to, then will call \codeword{update_...()}: Several methods behave very similarly. Activated by the buttons below except the finish button. They all start a subprocess to allow doctors to edit data and store it in their corresponding attrubute.
  \item \codeword{finish():} After all the data has been edited, press the finish button to call. First, the function will search for the corresponding patient based on the entered UID. If it is found, it will directly load the patient's information (\codeword{patient_info_t}). If the patient cannot be found, the program will first ask the doctor to create a file for the patient. Next, save all the entered data into a new \codeword{reports_t} object, then save \codeword{reports_t} into a new \codeword{med_record_t} object, append the data into the \codeword{patient_info_t} object just loaded or created, and finally save the updated \codeword{patient_info_t} to database
\end{itemize}
