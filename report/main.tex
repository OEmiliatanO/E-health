...
\end{figure}
The page firstly get input from textbox, and use \codeword{get\_info\_by\_id()} to call serials of methods in \codeword{patient\_info\_t} to load info of the patient the doctor wants to search. 
Then, it will call \codeword{display\_info()} to open a subprocess and pass the patient's info it gets. The subprocess can use a describe object to present these info.
\begin{figure}[h]
  \centering
...
\end{figure}
The describe object will firstly process the info by using \codeword{process\_info(String)}, put it into a suitable format and divide it into parts.
Then, it will present these information by using a scoll window, there are buttons to go prev and next pages, each pages present individual part of informations it just processed by \codeword{process\_info(String)}.
